...
async function saveData(day, type, value) {
    try {
        ...
        const response = await fetch(`http://localhost:5000/data?day=${day}`);
        const currentDayData = await response.json();

        // Si no existe, crear un nuevo objeto para ese día
        ...
        console.log("Datos enviados al servidor:", updatedDay); // Depuración

        // Enviar los datos actualizados al servidor
        const saveResponse = await fetch("http://localhost:5000/data", {
            method: "POST",
...
async function loadSavedData() {
    try {
        ...
        if (!Array.isArray(data)) {
            console.error("Los datos cargados no son un array:", data);
            return;
        }
